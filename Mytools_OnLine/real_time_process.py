...
class DataProcessor(th.Thread):
    def __init__(self, name, config, bin_queue):
        """
        :param name: str
                        Object name
        ...
        '''滤波器'''
        self.fs_b = 20  # 呼吸滤波器的采样率
        self.fs_h = 20  # 心跳滤波器的采样率
        ...
        for i in E_B['G_e_b']:
            temp_G = temp_G * i
        breath_b, self.breath_a = signal.sos2tf(E_B['SOS_e_b'])
        self.breath_b = breath_b * temp_G

        E_H = loadmat('E_H.mat')  # 同理E_H里面存储的是心跳滤波器的参数SOS_e_H和G_e_h
        temp_G = 1
        for i in E_H['G_e_h']:
            temp_G = temp_G * i
        heart_b, self.heart_a = signal.sos2tf(E_H['SOS_e_h'])
        self.heart_b = heart_b * temp_G
